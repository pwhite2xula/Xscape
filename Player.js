...
Platformer.Player = function (game_state, position, properties) {
    ...
    this.health = +localStorage.player_health || 25;
    ...
    this.animations.add("walking", [4,3,2,1,0], 10, true);
...
Platformer.Player.prototype.update = function () {
    ...
    if (this.cursors.right.isDown && this.body.velocity.x >= 0) {
        ...
        this.animations.play("walking");
        this.scale.setTo(1, 1);
    } else if (this.game_state.input.keyboard.isDown(Phaser.Keyboard.D) && this.body.velocity.x >= 0) {
        // move right
        this.body.velocity.x = this.walking_speed + 300;
        this.direction = "RIGHT";
        this.animations.play("walking");
        this.scale.setTo(-1, 1);
    } else if (this.game_state.input.keyboard.isDown(Phaser.Keyboard.A) && this.body.velocity.x <= 0) {
        // move left
        this.body.velocity.x = -this.walking_speed + 300;
        this.direction = "LEFT";
        this.animations.play("walking");
    ...
    if (this.game_state.input.keyboard.isDown(Phaser.Keyboard.SPACEBAR) && this.body.blocked.down) {
        this.body.velocity.y = -this.jumping_speed;
    }
    
    if (this.game_state.input.keyboard.isDown(Phaser.Keyboard.R) && this.game_state.input.keyboard.isDown(Phaser.Keyboard.E)) {
        this.game_state.game_over();
    }
    
    
    // if the player is able to shoot and the shooting button is pressed, start shooting
    if (this.shooting && this.game_state.input.keyboard.isDown(Phaser.Keyboard.SHIFT)) {
...
Platformer.Player.prototype.hit_enemy = function (player, enemy) {
    ...
    if (enemy.body.touching.up) {
        ...
        enemy.kill();
        player.y -= this.bouncing; // not needed
    } else {
        //this.die(enemy);
        //player.health = player.health - 1;
        this.health = localStorage.player_health - Math.floor(Math.random() * (enemy.max - enemy.min + 1)) + enemy.min;
    player.x = 35.0;
    player.y  = 664.0
    }
};
...
Platformer.Player.prototype.die = function (player, enemy) {
    "use strict";
   // this.shooting = false;
    
    this.health = localStorage.player_health - enemy.properties.max;
    player.x = 35.0;
    player.y  = 664.0
